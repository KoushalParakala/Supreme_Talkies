...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Fetch User and Profile to determine Name and Role
    const [userRes, profileRes] = await Promise.all([
      supabaseAdmin.auth.admin.getUserById(record.user_id),
      supabaseAdmin.from('profiles').select('full_name, role, niche').eq('id', record.user_id).single()
    ])

    if (userRes.error || !userRes.data.user) throw new Error('User not found')
    const user = userRes.data.user
    const profile = profileRes.data

    const user_email = user.email
    const full_name = profile?.full_name || user.user_metadata?.full_name || 'Creative'
    
    // Fallback logic for member's role in project
    const user_role = profile?.niche || profile?.role || 'Creative'
      ...
      <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #0e0f13; color: #F0EBE0; padding: 40px; text-align: center;">
        <div style="max-width: 600px; margin: 0 auto; border: 1px solid rgba(188,168,142,0.2); padding: 40px; background-color: #0a0b0e; text-align: left;">
          ...
          <div style="background: rgba(188,168,142,0.05); padding: 20px; border-left: 2px solid #BCA88E; margin-bottom: 30px;">
            ...
            <p style="margin: 0 0 15px; font-size: 12px; color: #F0EBE0; opacity: 0.6;">Role: ${user_role}</p>
