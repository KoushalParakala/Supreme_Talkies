        ...
        <>
          ...
          <div id="marketing-campaigns">
            <div style={{ marginBottom: 32 }}>
              <p style={{ fontFamily: 'DM Serif Display, serif', fontSize: 18, color: '#e8472a', letterSpacing: 2, marginBottom: 4 }}>CAMPAIGN CENTER</p>
                    ...
                    <div key={c.id} style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(23,23,23,0.12)', padding: 32 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                            <span style={{ 
                              width: 8, height: 8, borderRadius: '50%', background: c.status === 'active' ? '#e8472a' : '#666',
                              boxShadow: c.status === 'active' ? '0 0 10px #e8472a' : 'none',
                          ...
                          <div style={{ display: 'flex', gap: 8 }}>
                            {c.platform?.map((p: string) => (
                              <span key={p} style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 8, color: '#e8472a', opacity: 0.6, letterSpacing: 2 }}>
                                {p === 'Instagram' ? '📸' : p === 'Twitter/X' ? '🐦' : '🔗'} {p.toUpperCase()}
                              </span>
                            ))}
                          </div>
                        </div>
                        <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 9, letterSpacing: 3, border: '1px solid rgba(232,71,42,0.28)', padding: '4px 10px', color: '#e8472a' }}>{c.status.toUpperCase()}</span>
                      </div>
                      ...
                      <div style={{ marginBottom: 24 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                          ...
                          <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 9, letterSpacing: 2, color: '#e8472a' }}>{Math.round(progress)}%</p>
                        </div>
                        <div style={{ height: 2, background: 'rgba(23,23,23,0.12)', overflow: 'hidden' }}>
                          <motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }} style={{ height: '100%', background: '#e8472a' }} />
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 20, borderTop: '1px solid rgba(23,23,23,0.04)', gap: 16, flexWrap: 'wrap' }}>
                        {myAssignments[c.id] ? (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                            ...
                            <button onClick={() => handleLogPost(c.id)} style={{ background: 'transparent', border: '1px solid #e8472a', color: '#e8472a', padding: '6px 16px', fontFamily: 'Space Grotesk, sans-serif', fontSize: 9, letterSpacing: 3, cursor: 'pointer' }}>
                              + LOG POST
                            </button>
                          </div>
                        ) : (
                          <button onClick={() => handleJoinCampaign(c.id)} style={{ background: '#e8472a', border: 'none', color: '#fffdf7', padding: '8px 24px', fontFamily: 'Space Grotesk, sans-serif', fontSize: 9, fontWeight: 700, letterSpacing: 4, cursor: 'pointer' }}>
                            JOIN MISSION
                          </button>
                        )}
                        <button
                          ...
                          style={{ background: 'transparent', border: '1px solid rgba(232,71,42,0.4)', color: '#e8472a', padding: '6px 16px', fontFamily: 'Space Grotesk, sans-serif', fontSize: 9, letterSpacing: 3, cursor: 'pointer' }}
                        >
                          COPY KIT
                        </button>
                      </div>
          ...
          <div id="marketing-ideas" style={{ paddingTop: 32, borderTop: '1px solid rgba(23,23,23,0.12)' }}>
            <div style={{ marginBottom: 32 }}>
              <p style={{ fontFamily: 'DM Serif Display, serif', fontSize: 18, color: '#e8472a', letterSpacing: 2, marginBottom: 4 }}>CAMPAIGN IDEA BOARD</p>
          ...
          <div style={{ paddingTop: 64, borderTop: '1px solid rgba(23,23,23,0.12)' }}>
            ...
            <p style={{ fontFamily: 'DM Serif Display, serif', fontSize: 22, color: '#e8472a', letterSpacing: 2, marginBottom: 6 }}>THE COLLAB BRIEF</p>
