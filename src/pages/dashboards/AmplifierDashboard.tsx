...
function CinemaButton({ children, onClick, disabled, loading, style }: { 
  ...
  return (
    <motion.button type="button" onClick={onClick} disabled={disabled || loading}
      ...
      animate={{ background: hov && !disabled ? '#E47E44' : 'transparent', color: hov && !disabled ? '#fffdf7' : '#E47E44', opacity: disabled ? 0.4 : 1 }}
      transition={{ duration: 0.2 }}
      style={{ border: '1px solid #E47E44', padding: '13px 44px', fontFamily: 'DM Serif Display, serif', fontSize: 14, letterSpacing: 5, display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', ...style }}
    >
      ...
      <div style={{ display: 'flex', gap: 32, borderBottom: '1px solid rgba(23,23,23,0.12)', paddingBottom: 0 }}>
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
            style={{ 
              background: 'none', border: 'none', borderBottom: activeTab === tab.id ? '2px solid #E47E44' : '2px solid transparent',
              padding: '16px 0', fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, letterSpacing: 5,
              color: activeTab === tab.id ? '#E47E44' : 'rgba(228,126,68,0.28)', cursor: 'pointer', transition: 'all 0.3s'
            }}
          >
            {tab.label}
          </button>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'impact' && (
          <motion.div id="amp-impact" key="impact" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: 64 }}>
            ...
            {/* Story Streak */}
            <div style={{ textAlign: 'center', padding: '60px 40px', border: '1px solid rgba(228,126,68,0.1)', background: 'rgba(228,126,68,0.04)' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
                {streak > 0 ? (
                  <>
                    <span style={{ fontSize: 60 }}>🔥</span>
                    <h2 style={{ fontFamily: 'DM Serif Display, serif', fontSize: 72, color: '#E47E44', margin: 0, lineHeight: 1 }}>{streak}</h2>
                    <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: '#E47E44', letterSpacing: 8, margin: 0 }}>DAY STREAK</p>
                    <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 9, color: '#E47E44', letterSpacing: 3, margin: 0, opacity: 0.7 }}>Logged by you</p>
                    {profile?.last_share_at && <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 12, color: '#E47E44', opacity: 0.4, margin: 0 }}>Last share: {timeAgo(profile.last_share_at)}</p>}
                  </>
                ) : (
                ...
                <div style={{ marginTop: 40, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                  ...
                  <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: '#E47E44', opacity: 0.45, letterSpacing: 2, margin: 0 }}>
        ...
        {activeTab === 'shoutouts' && (
          <motion.div id="amp-wall" key="shoutouts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', flexDirection: 'column', gap: 48 }}>
            {/* Post Form */}
            <div style={{ border: '1px solid rgba(228,126,68,0.1)', padding: 32, background: 'rgba(247,245,239,1)', display: 'flex', flexDirection: 'column', gap: 24 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <label style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: '#E47E44', letterSpacing: 5 }}>MESSAGE</label>
                <textarea 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {shoutouts.map(sh => (
                <div key={sh.id} style={{ background: 'rgba(14,15,20,0.8)', border: '1px solid rgba(228,126,68,0.08)', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 11, color: '#E47E44' }}>{sh.likes || 0}</span>
