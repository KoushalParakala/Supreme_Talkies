...
function CinemaButton({ children, onClick, disabled, loading, style }: { 
  ...
  return (
    <motion.button type="button" onClick={onClick} disabled={disabled || loading}
      ...
      style={{ border: '1px solid var(--vermilion)', padding: '13px 44px', fontFamily: 'DM Serif Display, serif', fontSize: 14, letterSpacing: 5, display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', ...style }}
    >
      ...
      <div style={{ display: 'flex', gap: 32, borderBottom: '1px solid rgba(var(--ink-rgb),0.12)', paddingBottom: 0 }}>
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
            style={{ 
              background: 'none', border: 'none', borderBottom: activeTab === tab.id ? '2px solid var(--vermilion)' : '2px solid transparent',
              padding: '16px 0', fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, letterSpacing: 5,
              color: activeTab === tab.id ? 'var(--vermilion)' : 'color-mix(in srgb, var(--vermilion) 28%, transparent)', cursor: 'pointer', transition: 'all 0.3s'
            }}
          >
            {tab.label}
          </button>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'impact' && (
          <motion.div id="amp-impact" key="impact" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: 64 }}>
            ...
            {/* Story Streak */}
            <div style={{ textAlign: 'center', padding: '60px 40px', border: '1px solid color-mix(in srgb, var(--vermilion) 10%, transparent)', background: 'color-mix(in srgb, var(--vermilion) 4%, transparent)' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
                {streak > 0 ? (
                  <>
                    <span style={{ fontSize: 60 }}>🔥</span>
                    <h2 style={{ fontFamily: 'DM Serif Display, serif', fontSize: 72, color: 'var(--vermilion)', margin: 0, lineHeight: 1 }}>{streak}</h2>
                    <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: 'var(--vermilion)', letterSpacing: 8, margin: 0 }}>DAY STREAK</p>
                    <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 9, color: 'var(--vermilion)', letterSpacing: 3, margin: 0, opacity: 0.7 }}>Logged by you</p>
                    {profile?.last_share_at && <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 12, color: 'var(--vermilion)', opacity: 0.4, margin: 0 }}>Last share: {timeAgo(profile.last_share_at)}</p>}
                  </>
                ) : (
                  <div style={{ opacity: 0.3 }}>
                    <h2 style={{ fontFamily: 'DM Serif Display, serif', fontSize: 42, color: 'var(--vermilion)', margin: '0 0 12px' }}>0 DAYS</h2>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 32, flexWrap: 'wrap', justifyContent: 'center' }}>
                  {[
                    ...
                    <span key={m.days} style={{ 
                      padding: '6px 14px', border: '1px solid', borderColor: streak >= m.days ? 'var(--vermilion)' : 'rgba(var(--ink-rgb),0.12)',
                      color: streak >= m.days ? 'var(--ink)' : 'var(--vermilion)', opacity: streak >= m.days ? 1 : 0.2,
                ...
                <div style={{ marginTop: 40, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                  ...
                  <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: 'var(--vermilion)', opacity: 0.45, letterSpacing: 2, margin: 0 }}>
            ...
            <div>
              ...
              {profile?.is_early_access ? (
                <div style={{ border: '1px solid var(--vermilion)', background: 'rgba(var(--ink-rgb),0.04)', padding: 40 }}>
                  <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, letterSpacing: 8, color: 'var(--vermilion)', marginBottom: 12 }}>⚡ EARLY ACCESS MEMBER</p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {['Script DNA Marketplace', 'Live Screening Rooms', 'Director Matchmaking'].map(f => (
                      <div key={f} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(var(--ink-rgb),0.12)', paddingBottom: 12 }}>
                        ...
                        <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 8, letterSpacing: 2, background: 'rgba(var(--ink-rgb),0.12)', padding: '2px 8px', color: 'var(--vermilion)' }}>COMING SOON</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ border: '1px solid rgba(var(--ink-rgb),0.18)', padding: 40, opacity: 0.8 }}>
                  <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, letterSpacing: 8, color: 'var(--vermilion)', marginBottom: 12 }}>APPLY FOR EARLY ACCESS</p>
        ...
        {activeTab === 'shoutouts' && (
          <motion.div id="amp-wall" key="shoutouts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', flexDirection: 'column', gap: 48 }}>
            {/* Post Form */}
            <div style={{ border: '1px solid color-mix(in srgb, var(--vermilion) 10%, transparent)', padding: 32, background: 'rgba(247,245,239,1)', display: 'flex', flexDirection: 'column', gap: 24 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <label style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: 'var(--vermilion)', letterSpacing: 5 }}>MESSAGE</label>
                <textarea 
                  ...
                  style={{ background: 'transparent', border: 'none', borderBottom: '1px solid color-mix(in srgb, var(--vermilion) 28%, transparent)', color: 'var(--ink)', fontFamily: 'Space Grotesk, sans-serif', fontSize: 14, outline: 'none', resize: 'none' }}
                />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <label style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: 'var(--vermilion)', letterSpacing: 5 }}>LINK (OPTIONAL)</label>
                <input 
                  type="text" placeholder="Attach a link" value={shoutoutLink} onChange={e => setShoutoutLink(e.target.value)}
                  style={{ background: 'transparent', border: 'none', borderBottom: '1px solid color-mix(in srgb, var(--vermilion) 28%, transparent)', color: 'var(--ink)', fontFamily: 'Space Grotesk, sans-serif', fontSize: 13, outline: 'none' }}
                />
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {shoutouts.map(sh => (
                <div key={sh.id} style={{ background: 'rgba(14,15,20,0.8)', border: '1px solid color-mix(in srgb, var(--vermilion) 8%, transparent)', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <span style={{ fontSize: 24 }}>{sh.profiles?.avatar_symbol || '👤'}</span>
                      <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 11, color: 'var(--vermilion)', margin: 0, fontWeight: 700 }}>{sh.profiles?.full_name?.toUpperCase()}</p>
                      <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 8, color: 'var(--vermilion)', opacity: 0.4, background: 'rgba(var(--ink-rgb),0.12)', padding: '2px 6px' }}>{sh.profiles?.st_id ? (sh.profiles.st_id.startsWith('SUPR-') ? sh.profiles.st_id : 'SUPR-' + sh.profiles.st_id) : 'NO-ID'}</span>
                    </div>
                    <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: 'var(--vermilion)', opacity: 0.3, margin: 0 }}>{timeAgo(sh.created_at)}</p>
                  </div>
                  
                  <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 14, color: 'var(--ink)', lineHeight: 1.7, margin: 0 }}>{sh.message}</p>
                  
                  {sh.link && (
                    <a href={sh.link} target="_blank" rel="noopener noreferrer" style={{ alignSelf: 'flex-start', background: 'rgba(var(--ink-rgb),0.12)', color: 'var(--vermilion)', textDecoration: 'none', fontSize: 9, padding: '4px 10px', fontFamily: 'Space Grotesk, sans-serif', letterSpacing: 1 }}>{sh.link.substring(0, 30)}...</a>
                  )}

                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 11, color: 'var(--vermilion)' }}>{sh.likes || 0}</span>
