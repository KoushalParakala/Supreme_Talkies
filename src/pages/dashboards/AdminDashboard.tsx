...
export default function AdminDashboard() {
  ...
  const [newFilm, setNewFilm] = useState<any>({
    title: '', logline: '', color: '#0a0a0a', rating: 'UA', duration: '',
    director: '', producer: '', cast_members: '', synopsis: '', special_note: '',
    video_link: '', reel_image: '', poster_image: '', coming_soon: false
  });
  ...
  const saveFilm = async () => {
    try {
      ...
      if (posterFile) {
        ...
        const { error: uploadError } = await supabase.storage.from('cinematic_assets').upload(filePath, posterFile);
      ...
      if (reelFile) {
        const fileExt = reelFile.name.split('.').pop();
        const fileName = `${Math.random()}.${fileExt}`;
        const filePath = `reels/${fileName}`;
        const { error: uploadError } = await supabase.storage.from('cinematic_assets').upload(filePath, reelFile);
        if (uploadError) throw uploadError;
        const { data: { publicUrl } } = supabase.storage.from('cinematic_assets').getPublicUrl(filePath);
        finalReelUrl = publicUrl;
      }

      const filmPayload = { ...newFilm, poster_image: finalPosterUrl, reel_image: finalReelUrl };
      ...
      setPosterFile(null);
      setReelFile(null);
      setNewFilm({ title: '', logline: '', color: '#0a0a0a', rating: 'UA', duration: '', director: '', producer: '', cast_members: '', synopsis: '', special_note: '', video_link: '', reel_image: '', poster_image: '', coming_soon: false });
      fetchData();
    } catch (e: any) { 
      alert(e.message); 
    } finally {
                ...
                {scripts.map(s => (
                  <div key={s.id} style={{ padding: 24, border: '1px solid rgba(188,168,142,0.1)', background: 'rgba(0,0,0,0.2)', display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'flex-end' }}>
                      ...
                      {/* Move Buttons in List View */}
                      <div style={{ display: 'flex', gap: 16, marginTop: 12 }}>
                        {(() => {
                          const currentStageIdx = KANBAN_STAGES.findIndex(st => st.id === (s.kanban_stage || 'inbox'));
                          return (
                            <>
                              <button 
                                disabled={currentStageIdx === 0}
                                onClick={() => moveScriptStage(s.id, KANBAN_STAGES[currentStageIdx - 1].id)}
                                style={{ background: 'none', border: '1px solid rgba(188,168,142,0.2)', color: '#BCA88E', fontSize: 8, padding: '4px 8px', letterSpacing: 1, cursor: currentStageIdx === 0 ? 'default' : 'pointer', opacity: currentStageIdx === 0 ? 0.2 : 0.6 }}
                              >
                                ← BACK
                              </button>
                              <button 
                                disabled={currentStageIdx === KANBAN_STAGES.length - 1}
                                onClick={() => moveScriptStage(s.id, KANBAN_STAGES[currentStageIdx + 1].id)}
                                style={{ background: 'none', border: '1px solid rgba(188,168,142,0.2)', color: '#BCA88E', fontSize: 8, padding: '4px 8px', letterSpacing: 1, cursor: currentStageIdx === KANBAN_STAGES.length - 1 ? 'default' : 'pointer', opacity: currentStageIdx === KANBAN_STAGES.length - 1 ? 0.2 : 0.6 }}
                              >
                                FORWARD →
                              </button>
                            </>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        {stageScripts.map(script => (
                          ...
                          >
                            ...
                            {/* Move Buttons */}
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 14, paddingTop: 10, borderTop: '1px solid rgba(188,168,142,0.05)' }}>
                              <button 
                                disabled={idx === 0}
                                onClick={() => moveScriptStage(script.id, KANBAN_STAGES[idx - 1].id)}
                                style={{ background: 'none', border: 'none', color: '#BCA88E', fontSize: 8, letterSpacing: 2, cursor: idx === 0 ? 'default' : 'pointer', opacity: idx === 0 ? 0.2 : 0.6 }}
                              >
                                ← BACK
                              </button>
                              <button 
                                ...
                                onClick={() => moveScriptStage(script.id, KANBAN_STAGES[idx + 1].id)}
                                style={{ background: 'none', border: 'none', color: '#BCA88E', fontSize: 8, letterSpacing: 2, cursor: idx === KANBAN_STAGES.length - 1 ? 'default' : 'pointer', opacity: idx === KANBAN_STAGES.length - 1 ? 0.2 : 0.6 }}
                              >
                                FORWARD →
                              </button>
                            </div>
          ...
          <motion.div key="films" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
            ...
            <div style={{ padding: 32, border: '1px solid rgba(188,168,142,0.2)', background: 'rgba(188,168,142,0.03)', display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
                <CinemaInput label="COLOR (HEX)" value={newFilm.color} onChange={(v) => setNewFilm({ ...newFilm, color: v })} />
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
                ...
                {editingFilm && <button onClick={() => { setEditingFilm(null); setPosterFile(null); setReelFile(null); setNewFilm({ title: '', logline: '', color: '#0a0a0a', rating: 'UA', duration: '', director: '', producer: '', cast_members: '', synopsis: '', special_note: '', video_link: '', reel_image: '', poster_image: '', coming_soon: false }); }} style={{ background: 'none', border: '1px solid rgba(255,80,80,0.3)', color: '#ff5050', padding: '0 24px', fontSize: 10, cursor: 'pointer' }}>CANCEL</button>}
              </div>
            </div>
