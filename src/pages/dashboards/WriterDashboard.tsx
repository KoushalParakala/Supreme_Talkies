...
const STATUS_COLORS: Record<string, string> = {
  submitted:    'rgba(188,168,142,0.4)',
  under_review: 'rgba(100,160,255,0.5)',
  accepted:     'rgba(100,200,120,0.5)',
  feedback:     'rgba(255,160,80,0.5)',
};
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 32, paddingTop: 28, maxWidth: 680 }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                            {versionHistory.slice(0, 5).map((v, i) => (
                              <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 20, paddingBottom: 12, borderBottom: i < 4 ? '1px solid rgba(188,168,142,0.05)' : 'none' }}>
                                <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 10, fontWeight: 700, color: '#BCA88E', letterSpacing: 2 }}>V{v.version_number}</span>
                                <span style={{ fontFamily: 'Inter, monospace', fontSize: 9, color: '#BCA88E', opacity: 0.5, whiteSpace: 'nowrap' }}>
                                  {new Date(v.created_at).toLocaleDateString()}
                                </span>
                                <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: '#F0EBE0', opacity: 0.8, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                  {v.version_notes}
                                </p>
                              </div>
                            ))}
                            {versionHistory.length > 5 && (
                              <button style={{ background: 'none', border: 'none', color: '#BCA88E', fontFamily: 'Montserrat, sans-serif', fontSize: 8, letterSpacing: 4, cursor: 'pointer', textAlign: 'left', padding: 0, opacity: 0.6 }}>VIEW ALL VERSIONS</button>
                            )}
                          </div>
                        </div>
                      </motion.div>
              ...
              {loading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
                  {submissions.map((script: any) => (
                    <div key={script.id} style={{ borderBottom: '1px solid rgba(188,168,142,0.1)', paddingBottom: 24 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <p style={{ fontFamily: 'Playfair Display, sans-serif', fontSize: 13, color: '#BCA88E', letterSpacing: 4, opacity: 0.6, margin: 0 }}>{script.title}</p>
                        <span style={{ fontFamily: 'Inter, monospace', fontSize: 9, letterSpacing: 3, padding: '3px 8px', border: `1px solid ${STATUS_COLORS[script.status] || 'rgba(188,168,142,0.3)'}`, color: STATUS_COLORS[script.status] || '#BCA88E' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <motion.div
                          ...
                          style={{ background: 'rgba(255,255,255,0.02)', padding: '12px 16px' }}
                        >
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                              <span style={{ fontFamily: 'Inter, monospace', fontSize: 9, letterSpacing: 2, border: '1px solid rgba(188,168,142,0.25)', padding: '2px 6px', color: '#BCA88E' }}>
                                V{script.version_number || 1}
                              </span>
                              <p style={{ fontFamily: 'Inter, monospace', fontSize: 10, color: '#F0EBE0', opacity: 0.5, letterSpacing: 2 }}>
                                {new Date(script.updated_at || script.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }).toUpperCase()}
                              </p>
                              {script.kanban_stage && (
                                <span style={{ fontFamily: 'Inter, monospace', fontSize: 8, color: '#BCA88E', opacity: 0.4, letterSpacing: 1 }}>
                                  [{script.kanban_stage.toUpperCase()}]
                                </span>
                              )}
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                              <a href={script.pdf_url} target="_blank" rel="noopener noreferrer"
                                style={{ fontFamily: 'Inter, monospace', fontSize: 10, color: '#BCA88E', opacity: 0.5, letterSpacing: 2, textDecoration: 'underline' }}>
                                READ LATEST →
                              </a>
                            </div>
                          </div>
