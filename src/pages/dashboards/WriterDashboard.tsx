...
const STATUS_COLORS: Record<string, { text: string; glow: string; bg: string }> = {
  ...
  inbox:        { text: '#e8472a',  glow: 'rgba(232,71,42,0.18)', bg: 'rgba(232,71,42,0.08)' },
  submitted:    { text: '#e8472a',  glow: 'rgba(232,71,42,0.18)', bg: 'rgba(232,71,42,0.08)' },
...
function ScriptStatusTimeline({ stage }: { stage: string }) {
  ...
  const finalColor = normalized === 'rejected' ? '#F87171' : normalized === 'accepted' ? '#34D399' : '#e8472a';
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 0, flexWrap: 'wrap', marginTop: 4 }}>
      {steps.map((step, i) => {
        ...
        const color = step.id === 'final' && active ? finalColor : active || done ? '#e8472a' : 'rgba(232,71,42,0.18)';
        return (
          <div key={step.id} style={{ display: 'flex', alignItems: 'center' }}>
            ...
            {i < steps.length - 1 && (
              <div style={{
                ...
                background: i < currentIdx ? '#e8472a' : 'rgba(23,23,23,0.18)',
              }} />
            )}
          </div>
        );
...
const VERSION_PALETTE = ['#e8472a','#60A5FA','#FBBF24','#34D399','#F87171','#A78BFA','#FB923C','#E879F9'];
        ...
        {activeTab === 'scripts' && (
          ...
          >
            ...
            <div ref={submitFormRef}>
              ...
              <p style={{ fontFamily: 'DM Serif Display, serif', fontSize: 22, color: '#e8472a', letterSpacing: 2, marginBottom: 6 }}>THE SCRIPT PORTAL</p>
              ...
              <div style={{ marginTop: 32, display: 'flex', flexDirection: 'column', gap: 40 }}>
                ...
                <div style={{ borderTop: '1px solid rgba(23,23,23,0.12)', paddingTop: 32 }}>
                  <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, letterSpacing: 5, color: '#e8472a', marginBottom: 20 }}>YOUR SCRIPTS</p>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                  ...
                                  <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: '#e8472a', opacity: 0.5, margin: 0 }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                {script.pdf_url && (
                                  <a href={script.pdf_url} target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: '#e8472a', textDecoration: 'none', letterSpacing: 2 }}>
                                    READ SCRIPT
                                  </a>
                                )}
                                {script.status !== 'draft' && (
                                  <button
                                    ...
                                    style={{ background: 'none', border: '1px solid rgba(232,71,42,0.28)', color: '#e8472a', fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, padding: '4px 8px', cursor: 'pointer', letterSpacing: 1 }}
                                  >
                                    NEW REVISION
                                  </button>
                                )}
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              {openBriefs.length === 0 ? (
                ...
                return (
                  <div key={brief.id} style={{ background: '#f7f5ef', border: `1px solid ${hasInterested ? 'rgba(232,71,42,0.4)' : 'rgba(23,23,23,0.14)'}`, padding: 32, display: 'flex', flexDirection: 'column', gap: 20, transition: 'border-color 0.3s' }}>
                    ...
                    <div style={{ paddingTop: 16, borderTop: '1px solid rgba(23,23,23,0.12)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <CinemaButton 
                        ...
                        style={hasInterested ? { borderColor: '#e8472a', background: 'rgba(232,71,42,0.08)', padding: '10px 24px', fontSize: 12, letterSpacing: 3, whiteSpace: 'nowrap' } : { padding: '10px 24px', fontSize: 12, letterSpacing: 3, whiteSpace: 'nowrap' }}
                      >
        ...
        {activeTab === 'inspiration' && (
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 680, padding: 32, background: '#f7f5ef', border: '1px solid rgba(23,23,23,0.12)' }}>
              <p style={{ fontFamily: 'DM Serif Display, serif', fontSize: 18, color: '#e8472a', letterSpacing: 2, margin: 0 }}>ADD TO PINBOARD</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <p style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 9, color: '#e8472a', letterSpacing: 4, margin: 0 }}>TYPE</p>
                <div style={{ display: 'flex', gap: 12 }}>
                  {['LINK', 'QUOTE', 'FILM REF'].map(t => (
                    <button
                      ...
                      style={{
                        padding: '8px 16px',
                        background: pinForm.type === t ? 'rgba(232,71,42,0.1)' : 'none',
                        border: '1px solid',
                        borderColor: pinForm.type === t ? '#e8472a' : 'rgba(23,23,23,0.18)',
                        color: '#e8472a',
                        fontFamily: 'Space Grotesk, sans-serif',
                        fontSize: 8,
                        letterSpacing: 3,
              ...
              {inspirationPins.length === 0 ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 24 }}>
                  {inspirationPins.map(pin => (
                    <motion.div
                      key={pin.id}
                      whileHover={{ y: -4, borderColor: 'rgba(232,71,42,0.28)' }}
                      style={{
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <span style={{ fontSize: 8, fontFamily: 'Space Grotesk, sans-serif', letterSpacing: 3, color: '#e8472a', opacity: 0.6 }}>{pin.type}</span>
                        <button
                          onClick={() => handlePinDelete(pin.id)}
                          style={{
                            background: 'none', border: 'none', color: '#e8472a', cursor: 'pointer', padding: 4,
                      ...
                      {pin.url && (
                        <a 
                          ...
                          style={{ 
                            fontFamily: 'Space Grotesk, sans-serif', fontSize: 10, color: '#e8472a', 
